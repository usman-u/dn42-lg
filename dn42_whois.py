...
def hostname_regex(inp):
    if bool(re.match(r"^(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9])$", inp)):
        return True
    return False
...
def dn42_whois(target):
    ...
    return subprocess.run(["whois", "-h", "whois.dn42", target], check=True , capture_output=True, text=True).stdout
